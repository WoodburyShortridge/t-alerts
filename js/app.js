var round = document.getElementById('select-round-trip');
var one = document.getElementById('select-one-way');

function toggleAria(el) {
    var x = el.getAttribute("aria-pressed");
    if (x === "true") {
        x = "false"
    } else {
        x = "true"
    }
    el.setAttribute("aria-pressed", x);
}

round.onclick = function(event){
    event.preventDefault();

    var isActive = round.classList.contains("active");
    if (isActive === false) {
        round.classList.add("active");
        toggleAria(round)
    }

    isActive = one.classList.contains("active");
    if (isActive === true) {
...
one.onclick = function(event){
    event.preventDefault();
    var isActive = one.classList.contains("active");
    if (isActive === false) {
        one.classList.add("active");
        toggleAria(one)
    }

    isActive = round.classList.contains("active");
    if (isActive === true) {
